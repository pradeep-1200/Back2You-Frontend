...
const getStatusLabel = (status) => {
  if (status === 'lost') return 'Reported Lost';
  if (status === 'claimed') return 'Claimed';
  if (status === 'resolved') return 'Recovered';
  return 'Reported Found';
};

const getClaimUserId = (claim) => {
  if (!claim?.userId) return null;
  return typeof claim.userId === 'object' ? claim.userId._id : claim.userId;
};
...
const ItemDetail = () => {
  ...
  useEffect(() => {
    ...
    if (user && user.savedItems) {
      if (typeof user.savedItems[0] === 'object') {
        setIsSaved(user.savedItems.some((savedItem) => savedItem._id === id));
      } else {
        setIsSaved(user.savedItems.includes(id));
      }
    }

    fetchClaims().catch((err) => console.error(err));
  }, [id, user]);
  ...
  const handleUpdateClaim = async (claimId, status) => {
    ...
    try {
      ...
      toast.error(e.response?.data?.error || 'Failed to update claim');
    } finally {
  ...
  const handleSendMessage = async (claimId) => {
    ...
    try {
      ...
      setClaims((prevClaims) => prevClaims.map((claim) => (claim._id === claimId ? data : claim)));
      setChatMessage('');
    } catch (e) {
  ...
  const submitClaim = async () => {
    ...
    try {
      ...
      toast.error(e.response?.data?.error || "Failed submitting claim", { id: toastId });
    } finally {
      setClaiming(false);
    }
  };
  ...
  const userClaim = user ? claims.find((claim) => getClaimUserId(claim) === user._id) : null;
  const visibleClaims = claims.filter((claim) => user && (user._id === item.userId?._id || getClaimUserId(claim) === user._id));
        ...
        <motion.div initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="detail-info-box card">
          ...
          {item.status === 'resolved' && (
            <div className="resolution-panel">
              ...
              <p>{item.resolutionMessage || 'Item successfully returned'}</p>
              {item.resolvedAt && (
                <span className="resolution-meta">Resolved on {new Date(item.resolvedAt).toLocaleString()}</span>
              )}
              {item.matchedItem && (
                <Link to={`/item/${item.matchedItem._id}`} className="resolution-link">
                  View linked item: {item.matchedItem.title}
                </Link>
              )}
            </div>
          )}
          ...
          {userClaim && (
            <div className="resolution-panel" style={{ marginTop: '1rem' }}>
              <h3>Your Claim</h3>
              <p>You already submitted a claim for this item.</p>
              <span className="resolution-meta">Current status: {userClaim.status}</span>
            </div>
          )}

          {(!user || user._id !== item.userId?._id) && item.status === 'found' && !userClaim && (
            <motion.button
                    ...
                    {user._id === item.userId?._id && claim.status === 'pending' && (
                      <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
                        <button
                          ...
                          style={{ background: '#10b981', color: '#fff', border: 'none', borderRadius: '4px', padding: '0.3rem 0.6rem', cursor: updatingClaimId === claim._id ? 'not-allowed' : 'pointer', opacity: updatingClaimId === claim._id ? 0.7 : 1 }}
                        >
                          {updatingClaimId === claim._id ? 'Updating...' : 'Approve'}
                        </button>
                        <button
                          onClick={() => handleUpdateClaim(claim._id, 'rejected')}
                          disabled={updatingClaimId === claim._id}
                          style={{ background: '#ef4444', color: '#fff', border: 'none', borderRadius: '4px', padding: '0.3rem 0.6rem', cursor: updatingClaimId === claim._id ? 'not-allowed' : 'pointer', opacity: updatingClaimId === claim._id ? 0.7 : 1 }}
                        >
                          {updatingClaimId === claim._id ? 'Updating...' : 'Reject'}
                        </button>
                      </div>
                    )}
                    {user._id === item.userId?._id && claim.status === 'approved' && (
                      <button
                        onClick={() => handleUpdateClaim(claim._id, 'completed')}
                        disabled={updatingClaimId === claim._id}
                        style={{ background: '#3b82f6', color: '#fff', border: 'none', borderRadius: '4px', padding: '0.3rem 0.6rem', cursor: updatingClaimId === claim._id ? 'not-allowed' : 'pointer', opacity: updatingClaimId === claim._id ? 0.7 : 1 }}
                      >
                        {updatingClaimId === claim._id ? 'Updating...' : 'Mark Completed'}
                      </button>
                    )}
                  </div>
                </div>
